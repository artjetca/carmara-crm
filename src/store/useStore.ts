...
import { Customer, Visit, Profile, Prospect } from '../lib/supabase'
...
interface AppState {
  ...
  updateProspectInStore: (id: string, prospect: Partial<Prospect>) => void
  removeProspectFromStore: (id: string) => void
  setLoadingProspects: (loading: boolean) => void
}
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      updateProspectInStore: (id, update) => set((state) => ({
        prospects: state.prospects.map(p => p.id === id ? { ...p, ...update } : p)
      })),
      removeProspectFromStore: (id) => set((state) => ({
        prospects: state.prospects.filter(p => p.id !== id)
      })),
      setLoadingProspects: (loading) => set({ loadingProspects: loading })
    }),
    {
